...
const enhancements = [
  ...
  { title: 'QR-Based Student Verification', icon: <QrCode className="w-6 h-6" />, color: 'bg-blue-100 text-blue-600' },
  ...
  { title: 'Real-Time Notifications', icon: <Bell className="w-6 h-6" />, color: 'bg-indigo-100 text-indigo-600' }
];
    ...
    <section className="py-24 bg-white relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-indigo-50 text-indigo-700 text-sm font-bold mb-4">
            <Sparkles className="w-4 h-4" /> Roadmap
          </div>
          <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 mb-6">Future <span className="text-blue-600">Enhancements</span></h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {enhancements.map((item, index) => (
            <div key={index} className="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm hover:shadow-xl hover:border-blue-100 transition-all group">
