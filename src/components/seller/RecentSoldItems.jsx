        ...
        {salesData.length === 0 ? (
          ...
          <div className="space-y-4">
            {salesData.map((item, i) => (
              <div key={item._id || i} className="flex flex-col md:flex-row gap-4 p-4 rounded-xl border border-slate-100 bg-slate-50 hover:bg-white hover:border-theme-maroon/30 hover:shadow-md transition-all group">
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between">
                  <div className="flex justify-between items-start gap-4">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs font-medium text-slate-500">Sold to: <span className="font-bold text-slate-700">{item.buyerId?.name || 'Unknown'}</span></span>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 mt-3 pt-3 border-t border-slate-200/60">
                    <div className="flex items-center gap-4 text-xs font-bold text-slate-500">
                      <div className="flex items-center gap-1.5" title="Total Views"><Eye className="w-4 h-4 text-slate-400" /> {item.productId?.views || 0}</div>
                      <div className="flex items-center gap-1.5" title="Wishlist Saves"><Heart className="w-4 h-4 text-rose-400" /> {item.productId?.wishlistCount || 0}</div>
                    </div>
                    <div className="flex items-center gap-2">
                      <button className="p-1.5 text-slate-400 hover:text-theme-maroon hover:bg-theme-maroon/10 rounded-md transition-colors" title="View Details"><Eye className="w-4 h-4" /></button>
                      <button className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors" title="Contact Buyer"><MessageSquare className="w-4 h-4" /></button>
                      <button className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-md transition-colors" title="View Transaction"><CheckCircle className="w-4 h-4" /></button>
                      <button className="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-md transition-colors" title="View Buyer Rating"><Star className="w-4 h-4" /></button>
                    </div>
                  </div>
                </div>
              </div>
