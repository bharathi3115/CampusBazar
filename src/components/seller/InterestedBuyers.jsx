        ...
        <div className="space-y-4">
          {buyers.length === 0 ? (
            ...
            buyers.map((buyer) => (
              <div
                key={buyer.id}
                className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 rounded-xl border border-slate-100 bg-slate-50 hover:bg-white hover:shadow-md transition-all group">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 mt-1">
                      ...
                      <span className="text-xs font-medium text-slate-500 flex items-center gap-1">
                        <Clock className="w-3 h-3" /> Last contact: {buyer.lastContact}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2 w-full sm:w-auto mt-2 sm:mt-0">
                  <button
                    onClick={() => handleMessageClick(buyer.conversationId)}
                    className="flex-1 sm:flex-none px-4 py-2 bg-theme-maroon text-white text-sm font-bold rounded-lg shadow-sm hover:bg-theme-dark-maroon transition-colors flex items-center justify-center gap-2">
                    <MessageSquare className="w-4 h-4" /> Message
                  </button>
                  <button
                    onClick={() => setSelectedBuyer(buyer)}
                    className="flex-1 sm:flex-none px-4 py-2 bg-slate-100 text-slate-700 text-sm font-bold rounded-lg hover:bg-slate-200 transition-colors flex items-center justify-center gap-2">
      ...
      {selectedBuyer && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
            <button onClick={() => setSelectedBuyer(null)} className="absolute top-4 right-4 p-2 bg-white/50 hover:bg-white rounded-full text-slate-500 hover:text-slate-900 transition-colors z-10">
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="bg-gradient-to-br from-theme-maroon to-theme-dark-maroon p-8 flex flex-col items-center justify-center relative overflow-hidden text-center">
              ...
              <img src={selectedBuyer.avatar} alt={selectedBuyer.name} className="w-24 h-24 rounded-full border-4 border-white shadow-xl mb-4 relative z-10 object-cover bg-white" />
              <h2 className="text-2xl font-black text-white relative z-10">{selectedBuyer.name}</h2>
              <p className="text-theme-maroon-50 font-medium text-sm flex items-center gap-1 relative z-10 opacity-90 mt-1">{selectedBuyer.profile?.department || "Student"}</p>
            </div>
              ...
              <div className="border-t border-slate-100 pt-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-bold text-slate-900">{selectedBuyer.profile?.createdAt ? new Date(selectedBuyer.profile.createdAt).toLocaleDateString() : "Recently"}</span>
                  </div>
                </div>
              </div>
