        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-2xl shadow-2xl transform transition-all">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => { setIsCameraOpen(false); setCapturedImage(null); }} className="p-2 text-slate-400 hover:text-slate-600 bg-slate-100 rounded-full transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
        ...
        <div className="xl:col-span-2 space-y-8">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
              ...
              <div className="text-sm font-bold text-theme-maroon bg-theme-maroon/10 px-3 py-1.5 rounded-lg border border-theme-maroon/20">
                {images.length}/5 Uploaded
              </div>
            </div>

            <div className="flex flex-col sm:flex-row gap-4 mb-6">
              <button onClick={() => fileInputRef.current?.click()} className="flex-1 bg-slate-50 border border-slate-200 text-slate-700 font-bold py-3 px-4 rounded-xl hover:bg-slate-100 hover:border-slate-300 transition-all flex items-center justify-center gap-2 shadow-sm">
                <span className="text-xl">📁</span> Upload Images
              </button>
              <button onClick={() => setIsCameraOpen(true)} className="flex-1 bg-theme-maroon text-white font-bold py-3 px-4 rounded-xl shadow-lg shadow-theme-maroon/20 hover:bg-theme-dark-maroon transition-all flex items-center justify-center gap-2">
                ...
                {images.map((img, index) => (
                  <div key={index} className="relative group aspect-square rounded-xl overflow-hidden border border-slate-200 shadow-sm animate-in fade-in zoom-in duration-300">
                    ...
                    <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 backdrop-blur-sm">
                      {index !== 0 && (
                        <button 
                          onClick={(e) => { e.stopPropagation(); makeCoverImage(index); }}
                    ...
                    {index === 0 && (
                      <div className="absolute top-2 left-2 bg-theme-maroon text-white text-[10px] font-bold px-2 py-0.5 rounded shadow">
                        COVER
                      </div>
                    )}
                  </div>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 focus:border-theme-maroon focus:ring-2 focus:ring-theme-maroon/20 rounded-xl px-4 py-2.5 outline-none transition-all font-medium resize-none"
                ></textarea>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1.5">Selling Price (₹) <span className="text-red-500">*</span></label>
                  <input 
                    type="number" 
                    name="price"
                ...
                <div className="flex items-center gap-3 pt-2">
                  <div className="relative inline-block w-10 mr-2 align-middle select-none transition duration-200 ease-in">
                    ...
                    />
                    <label htmlFor="negotiable" className={`toggle-label block overflow-hidden h-5 rounded-full cursor-pointer transition-colors duration-200 ease-in-out ${formData.negotiable ? 'bg-theme-maroon' : 'bg-slate-300'}`}></label>
                  </div>
                  <label htmlFor="negotiable" className="text-sm font-bold text-slate-700 cursor-pointer">Price is negotiable</label>
                </div>
              </div>
            </div>
                  ...
                  <div 
                    ...
                    className={`p-3 border rounded-xl cursor-pointer text-center transition-all ${
                      formData.condition === cond 
                        ? 'border-theme-maroon bg-theme-maroon/5 ring-1 ring-theme-maroon' 
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-lg overflow-hidden transition-all hover:shadow-xl group">
                <div className="aspect-[4/3] bg-slate-100 relative overflow-hidden flex items-center justify-center">
                  ...
                  <div className="absolute top-3 left-3 flex flex-col gap-2">
                    <span className="bg-white/90 backdrop-blur-sm text-slate-800 text-xs font-black px-2.5 py-1 rounded-lg shadow-sm uppercase tracking-wider">
                      {formData.category || 'Category'}
                    </span>
                  </div>
                  {formData.condition && (
                    <div className="absolute top-3 right-3">
                      <span className="bg-theme-maroon text-white text-[10px] font-bold px-2 py-1 rounded-md shadow-sm uppercase tracking-wider">
                        {formData.condition}
                      </span>
                    </div>
                  )}
                ...
                <div className="p-5">
                  <div className="flex justify-between items-start mb-2 gap-2">
                    <h3 className="font-extrabold text-lg text-slate-900 leading-tight line-clamp-2">
                  ...
                  <div className="flex items-end gap-2 mb-4">
                    ...
                    {formData.originalPrice && (
                      <span className="text-sm text-slate-400 font-bold line-through mb-1">₹{formData.originalPrice}</span>
                    )}
                    {formData.negotiable && (
                      <span className="text-[10px] font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded ml-auto mb-1 border border-green-100">Negotiable</span>
                    )}
                  </div>
