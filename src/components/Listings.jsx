    ...
    <section className="py-20 bg-slate-50" id="items">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:justify-between md:items-end mb-12 gap-4">
          <div>
            <h2 className="text-3xl md:text-4xl font-bold text-slate-900 mb-4">
              Find Your <span className="text-purple-600">Item</span>
            </h2>
          ...
          <button className="hidden md:flex items-center gap-2 text-purple-600 font-semibold hover:text-purple-800 transition-colors">View All Listings &rarr;</button>
        </div>
          ...
          {listings.map((item) => (
            <div key={item.id} className="bg-white rounded-2xl border border-slate-100 overflow-hidden hover:shadow-xl transition-all duration-300 group flex flex-col">
              ...
              <div className="p-5 flex flex-col flex-grow">
                ...
                <h3 className="font-bold text-slate-900 text-base leading-tight mb-2 line-clamp-2 group-hover:text-purple-600 transition-colors">{item.title}</h3>
        ...
        <div className="mt-10 text-center md:hidden">
          <button className="bg-white border-2 border-slate-200 text-slate-900 font-bold py-3 px-8 rounded-full hover:bg-slate-50 transition-colors w-full">View All Listings</button>
        </div>
      </div>
    </section>
  );
