                ...
                reports.map((report) => (
                  <tr key={report._id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-6 py-4">
                      ...
                      <span
                        className={`inline-flex items-center gap-1 px-2 py-0.5 mt-1 rounded text-[10px] font-bold uppercase tracking-wider ${
                          report.severity === "High" ? "bg-red-100 text-red-700" : report.severity === "Medium" ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-700"
                        }`}>
                    ...
                    <td className="px-6 py-4 font-medium text-slate-600">{report.reporter?.email || report.reporter?.name || "Unknown"}</td>
                    <td className="px-6 py-4 text-slate-500 font-medium">{new Date(report.createdAt).toLocaleDateString()}</td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2 transition-opacity">
                        <button onClick={() => setSelectedProduct(report.productId)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="View Report Details">
                          <Eye className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleRemoveListing(report.productId, report._id)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Remove Listing">
