  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col md:flex-row transform transition-all">
        ...
        <button onClick={onClose} className="absolute top-4 right-4 z-10 p-2 bg-white/50 backdrop-blur-md text-slate-900 hover:bg-white rounded-full shadow-md transition-colors">
          <X className="w-5 h-5" />
        </button>

        {/* Left: Image Gallery */}
        <div className="w-full md:w-1/2 bg-slate-100 flex-shrink-0 relative h-64 md:h-auto">
          <img src={product.img || getFallbackImage(product.category, product.title)} alt={product.title} className="w-full h-full object-cover" />
          {product.condition === "New" && <div className="absolute top-6 left-6 bg-green-500 text-white text-xs font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg shadow-lg">Brand New</div>}
        </div>
        ...
        <div className="w-full md:w-1/2 flex flex-col overflow-y-auto">
          <div className="p-6 sm:p-8 flex-1">
            ...
            <div className="flex justify-between items-start gap-4 mb-2">
              <span className="text-xs font-bold text-theme-maroon uppercase tracking-wider bg-theme-maroon/5 px-2 py-1 rounded-md">{product.category}</span>
              <div className="flex items-center gap-3 text-slate-400 text-xs font-bold">
                <span className="flex items-center gap-1">
                  <Eye className="w-3.5 h-3.5" /> {product.views} Views
                </span>
                <span className="flex items-center gap-1">
                  <Heart className="w-3.5 h-3.5" /> {product.wishlistCount} Saves
                </span>
              </div>
            </div>

            <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 mb-4 leading-tight">{product.title}</h2>

            <div className="flex items-end gap-3 mb-6 pb-6 border-b border-slate-100">
              <span className="text-4xl font-black text-slate-900">₹{product.price}</span>
              {product.originalPrice && <span className="text-lg font-bold text-slate-400 line-through mb-1">₹{product.originalPrice}</span>}
            </div>
            ...
            <div className="mb-8">
              ...
              <p className="text-slate-600 leading-relaxed text-sm">
                {product.description ||
                  `Excellent condition ${product.title.toLowerCase()}. Barely used and well maintained. Selling because I no longer need it for my current semester. Perfect for anyone looking for a reliable item at a great price.`}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8 bg-slate-50 p-4 rounded-2xl border border-slate-100">
              ...
              <div>
                ...
                <span className="text-sm font-bold text-slate-900 flex items-center gap-1">
                  <Clock className="w-3.5 h-3.5 text-slate-400" /> {new Date(product.createdAt).toLocaleDateString()}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-2xl mb-8 hover:border-theme-maroon/30 transition-colors cursor-pointer group">
              <div className="flex items-center gap-3">
                <img src={getSafeAvatarUrl(product.seller?.avatarUrl, product.seller?.name)} alt={product.seller?.name} className="w-12 h-12 rounded-full bg-slate-100 border border-slate-200" />
                <div>
              ...
              {isReporting && (
                <div className="absolute bottom-full right-0 mb-2 w-64 bg-white border border-slate-200 rounded-2xl shadow-xl p-4 z-50">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setIsReporting(false)} className="flex-1 py-1.5 text-xs font-bold text-slate-500 hover:bg-slate-100 rounded-lg transition-colors">
                      Cancel
                    </button>
                    <button
