...
const BuyerDashboard = () => {
  ...
  const navItems = [
    ...
    { id: "messages", label: "Messages", icon: MessageSquare, badge: unreadMessages > 0 ? <span className="flex w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse"></span> : null },
    ...
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden">
      ...
      <aside className="w-64 bg-white border-r border-slate-200 hidden md:flex flex-col flex-shrink-0">
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between p-3 rounded-xl transition-all ${
                  isActive ? "bg-theme-maroon/10 text-theme-maroon font-bold" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900 font-medium"
                }`}>
                <div className="flex items-center gap-3">
                  <Icon className={`w-5 h-5 ${isActive ? "text-theme-maroon" : "text-slate-400"}`} />
                  <span>{item.label}</span>
                </div>
                {item.badge && <div className="flex items-center justify-center">{item.badge}</div>}
              </button>
            );
          })}
        </nav>
        ...
        <div className="p-4 border-t border-slate-100">
          <button onClick={logout} className="w-full flex items-center gap-3 p-3 rounded-xl font-medium text-slate-500 hover:bg-red-50 hover:text-red-600 transition-all">
        ...
        <header className="py-4 bg-white border-b-2 border-slate-200 px-6 sm:px-8 flex flex-col sm:flex-row sm:items-center justify-between flex-shrink-0 gap-4">
          ...
          <div className="flex items-center gap-3 sm:gap-5">
            <div className="relative pl-2 sm:pl-4 sm:border-l border-slate-200">
              <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setShowRoleMenu(!showRoleMenu)}>
                <img src={getSafeAvatarUrl(user?.avatarUrl, user?.name, user?.email)} alt="Profile" className="w-10 h-10 rounded-full bg-slate-100 border border-slate-200 object-cover" />
